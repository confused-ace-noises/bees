...
use syn::{Token, parse::Parse, parse_macro_input, punctuated::Punctuated, token::Type};

pub(crate) fn chain_impl(Chain { members }: Chain) -> syn::Result<TokenStream> {
    if members.len() < 2 {
        return Err(syn::Error::new(Span::call_site(), "chain! only accepts 2 or more arguments"));
    }

    let mut iter = members.into_iter().rev();

    // ? can be unwrapped because it's checked before
    let ChainMember { ty: last, try_token: last_try_token } = iter.next().unwrap();
    let ChainMember{ ty: second_to_last, try_token: first_try_token } = iter.next().unwrap(); 

    if let Some(token) = last_try_token {
        return Err(syn::Error::new_spanned(token, "Cannot use the ~ or try operator on the last `Handler`, as there is nothing to chain it to"));
    }
...
pub(crate) fn pipe_impl(Pipe { members }: Pipe) -> syn::Result<TokenStream> {
    if members.len() < 2 {
        return Err(syn::Error::new(Span::call_site(), "pipe! only accepts 2 or more arguments"));
    }
